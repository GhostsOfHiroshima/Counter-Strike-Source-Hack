...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* display_format, float power )
{
	...
	RenderTextClipped( frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), value_buf, value_buf_end, NULL, ImVec2( 0.5f, 0.5f ) );
...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	const ImRect text_bb( window->DC.CursorPos + ImVec2( 0, style.FramePadding.y ) - ImVec2( 0, 3 ), window->DC.CursorPos + ImVec2( 0, style.FramePadding.y ) + label_size );
	if ( label_size.x > 0 )
	{
